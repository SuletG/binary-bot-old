...
Blockly.Blocks.get_index_when = {
    init: function init() {
        this.appendDummyInput().appendField(translate('Get the index when a value from'));
        ...
        this.setTooltip(
            translate(
                'Return the index of the value that meets the condition. It will return -1 if it doesn\'t finds any, with 1 being the most recent index'
            )
        );
    },
...
Blockly.JavaScript.get_index_when = block => {
    ...
    return [
        `Bot.getIndexWhen({values: ${input}, condition: '${condition}', value: ${value}})`,
