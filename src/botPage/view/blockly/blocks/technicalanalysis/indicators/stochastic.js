...
Blockly.Blocks[mod] = {
    init: function init() {
        ...
        this.appendValueInput('PERIODSIGNAL')
            .setCheck('Number')
            .setAlign(Blockly.ALIGN_RIGHT)
        ...
        this.setTooltip(
            translate('Calculates the Stochastic (Stoch) from a list of candles with a period and a signal period')
        );
        if (this.isInFlyout) {
...
Blockly.JavaScript[mod] = block => {
    ...
    return [
        `Bot.${mod}({period:${period}, signalPeriod:${signalPeriod}}, ${input}, '${field}', '${returnType}')`,
