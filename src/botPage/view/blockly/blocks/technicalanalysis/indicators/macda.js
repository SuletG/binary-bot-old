...
Blockly.Blocks.macda = {
    init: function init() {
        this.appendDummyInput()
            .appendField(translate('MACD'))
            .appendField(new Blockly.FieldDropdown(config.macdFields), 'MACDFIELDS_LIST')
            .appendField(new Blockly.FieldDropdown(config.returnList), 'RETURNMACD');
        ...
        this.appendValueInput('FAST_EMA_PERIOD')
            .setCheck('Number')
            .setAlign(Blockly.ALIGN_RIGHT)
            .appendField(`${translate('Fast MA Period')}:`);
        this.appendValueInput('SLOW_EMA_PERIOD')
            .setCheck('Number')
            .setAlign(Blockly.ALIGN_RIGHT)
            .appendField(`${translate('Slow MA Period')}:`);
        this.appendValueInput('SIGNAL_EMA_PERIOD')
            .setCheck('Number')
            .setAlign(Blockly.ALIGN_RIGHT)
            .appendField(`${translate('Signal MA Period')}:`);
        this.appendDummyInput()
            .setAlign(Blockly.ALIGN_RIGHT)
            .appendField(`${translate('Use SMA for Oscillator')}`)
            .appendField(new Blockly.FieldCheckbox('false'), 'SIMPLE_MA_OSCILLATOR');
        this.appendDummyInput()
            .setAlign(Blockly.ALIGN_RIGHT)
            .appendField(`${translate('Use SMA for Signal')}`)
            .appendField(new Blockly.FieldCheckbox('false'), 'SIMPLE_MA_SIGNAL');
...
Blockly.JavaScript.macda = block => {
    ...
    const SimpleMAOscillator =
        Blockly.JavaScript.valueToCode(block, 'SIMPLE_MA_OSCILLATOR', Blockly.JavaScript.ORDER_ATOMIC) || 'false';
    const SimpleMASignal =
        Blockly.JavaScript.valueToCode(block, 'SIMPLE_MA_SIGNAL', Blockly.JavaScript.ORDER_ATOMIC) || 'false';
    const returnType = block.getFieldValue('RETURNMACD');
    return [
        `Bot.macd({values:${values},fastPeriod:${fastEmaPeriod},slowPeriod:${slowEmaPeriod},signalPeriod:${signalEmaPeriod},SimpleMAOscillator:${SimpleMAOscillator},SimpleMASignal:${SimpleMASignal}},'${macdField}','${returnType}')`,
