...
Blockly.Blocks[mod] = {
    init: function init() {
        ...
        this.appendValueInput('RSIPERIOD')
            .setCheck('Number')
            .setAlign(Blockly.ALIGN_RIGHT)
            .appendField(`${translate('RSI Period')}:`);
        this.appendValueInput('STOCHPERIOD')
            .setCheck('Number')
            .setAlign(Blockly.ALIGN_RIGHT)
            .appendField(`${translate('Stochastic Period')}:`);
        this.appendValueInput('KPERIOD')
            .setCheck('Number')
            .setAlign(Blockly.ALIGN_RIGHT)
            .appendField(`${translate('K Period')}:`);
        this.appendValueInput('DPERIOD')
            .setCheck('Number')
            .setAlign(Blockly.ALIGN_RIGHT)
            .appendField(`${translate('D Period')}:`);
        this.setOutput(true, 'Number');
        this.setColour(theme.subBlockColor);
        this.setTooltip(
            translate('Calculates the Stochastic RSI (StochasticRSI) from a list with a period and a signal period')
        );
        if (this.isInFlyout) {
...
Blockly.JavaScript[mod] = block => {
    ...
    const stochasticPeriod =
        Blockly.JavaScript.valueToCode(block, 'STOCHPERIOD', Blockly.JavaScript.ORDER_ATOMIC) || '14';
    ...
    return [
        `Bot.${mod}({values:${values}, rsiPeriod:${rsiPeriod}, stochasticPeriod:${stochasticPeriod}, kPeriod:${kPeriod}, dPeriod:${dPeriod}}, '${field}', '${returnType}')`,
