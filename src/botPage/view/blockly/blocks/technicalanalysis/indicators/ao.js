...
Blockly.Blocks[mod] = {
    init: function init() {
        ...
        this.appendValueInput('FASTPERIOD')
            .setCheck('Number')
            .setAlign(Blockly.ALIGN_RIGHT)
            .appendField(`${translate('Fast Period')}:`);
        this.appendValueInput('SLOWPERIOD')
            .setCheck('Number')
            .setAlign(Blockly.ALIGN_RIGHT)
            .appendField(`${translate('Slow Period')}:`);
        this.setOutput(true, 'Number');
        this.setColour(theme.subBlockColor);
        this.setTooltip(
            translate('Calculates the Awesome Oscillator (AO) from a list of candles with fast and low periods')
        );
        if (this.isInFlyout) {
...
Blockly.JavaScript[mod] = block => {
    ...
    return [
        `Bot.${mod}({fastperiod:${fastperiod}, slowperiod:${slowperiod}}, ${input}, '${returnType}')`,
